...
C++ 编译结果缓存在 `.localalgo/cache/cpp/`。重复运行相同源码和测试集时会
直接复用；修改源码、切换测试集、升级编译器或切换平台会自动生成新缓存。缓存
最多保留 64 项，可以随时安全删除。一个测试集只编译一次，但每个 case 会在
独立进程运行，避免全局变量、崩溃或超时影响其他 case。
...
还会检查函数名、参数类型和整数范围，不会静默覆盖已有题目。包含多道题的导入
会作为一个文件原子写入；单个自定义题目文件损坏时会跳过并报告，不影响其他
本地题目继续载入。
...
下一阶段适合增加更多真正不同的精选题、交互式 `/browse` 和可选的容器沙箱。
